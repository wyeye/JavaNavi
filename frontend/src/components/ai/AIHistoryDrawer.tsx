    ...
    return (
        ...
        >
            ...
            <div style={{ padding: '16px 16px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 14, fontWeight: 600, color: textColor }}>{t('ai.history.title')}</span>
                <Tooltip title={t('ai.history.collapseTooltip')}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '0 10px 16px' }} className="ai-history-list">
                {filteredSessions.length === 0 ? (
                    <div style={{ padding: '30px 0', textAlign: 'center', color: mutedColor, fontSize: 12 }}>{t('ai.history.noMatch')}</div>
                ) : (
                        ...
                        >
                            <div style={{ overflow: 'hidden', flex: 1, paddingRight: 8 }}>
                                <div style={{ fontSize: 13, color: textColor, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontWeight: sessionId === session.id ? 600 : 'normal' }}>
                                    {session.title || t('ai.chat.newSession')}
                                </div>
                            ...
                            <Tooltip title={t('ai.history.deleteTooltip')}>
                                <Button
