...
- Java-backed AI provider transport rejects localhost/private/link-local provider endpoints unless `JAVANAVI_ALLOW_PRIVATE_AI_ENDPOINTS=true` is set. Packaged desktop sidecars no longer set this by default; trusted local/LAN providers require explicit desktop opt-in with `JAVANAVI_DESKTOP_ALLOW_PRIVATE_AI_ENDPOINTS=true`, which maps to the backend variable for that sidecar process only.
